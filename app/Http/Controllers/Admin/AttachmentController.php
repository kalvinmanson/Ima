...
class AttachmentController extends Controller
{
    ...
    public function store(Request $request)
    {
      ...
      $filename = Str::slug(explode('.', $request->file('upload')->getClientOriginalName())[0]).'.'.$request->file('upload')->getClientOriginalExtension();
      if(Attachment::where('user_id', Auth::user()->id)->where('name', $filename)->first()) {
        $filename = Str::slug(explode('.', $request->file('upload')->getClientOriginalName())[0]).rand(10000,99999).'.'.$request->file('upload')->getClientOriginalExtension();
      }
      //create folder
